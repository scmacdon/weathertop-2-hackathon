...
Use Case:
  Use this script when you want to decommission a development or testing ECS cluster entirely, ensuring that
  no services, tasks, or task definitions remain.

What it does:
  1. Stops all running ECS tasks in the specified cluster.
  2. Stops and deletes all ECS services (scales them down to 0 first).
  3. Deregisters all task definition revisions for the specified task family.
  4. Deletes the ECS cluster.

Prerequisites:
  - AWS credentials must be configured via environment variables, IAM role, or AWS config file.
  - The boto3 library must be installed (`pip install boto3`).
  - You must have sufficient IAM permissions to manage ECS resources.

Variables to Configure:
  - `AWS_REGION`: AWS region where the ECS cluster resides.
  - `CLUSTER_NAME`: Name of the ECS cluster to clean up.
  - `TASK_DEFINITION_NAME`: Task definition family name to deregister.
...
TASK_DEFINITION_NAME = "WeathertopKotlin"
...
def stop_and_delete_services():
    ...
    # Set desired count to 0 to stop running tasks for each service
