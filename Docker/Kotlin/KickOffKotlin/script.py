...
def run_fargate_task():
    print(f"🚀 Launching Kotlin task '{TASK_DEFINITION}' in cluster '{CLUSTER_NAME}'...")
    try:
        ...
        print(f"✅ Started Kotlin task with ARN: {task_arn}")
        return task_arn
...
def wait_for_task_stop(cluster, task_arn, timeout_seconds=3600):
    ...
    while time.time() - start_time < timeout_seconds:
        try:
            ...
            print(f"❌ Exception checking task status: {e}")
            return None
...
if __name__ == "__main__":
    ...
    # Fetch logs
    log_stream_name = get_latest_log_stream(LOG_GROUP, f"{LOG_STREAM_PREFIX}/{CONTAINER_NAME}/")
    if log_stream_name:
