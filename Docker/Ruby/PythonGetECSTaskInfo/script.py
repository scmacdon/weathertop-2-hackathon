...
CLUSTER_NAME = 'MyNetWeathertopCluster'
TASK_DEFINITION_FAMILY = 'WeathertopNet'
LOG_GROUP = 'WeathertopContainerLogs'
EVENTBRIDGE_RULE_NAME = 'ecs-dotnet-schedule'
...
def check_eventbridge_ecs_permission(role_arn):
    """Check if the given IAM role has ecs:RunTask permissions."""
    if not role_arn:
        return False, "No RoleArn found for EventBridge target."

    role_name = role_arn.split("/")[-1]
    has_ecs_permission = False
    ...
    try:
        ...
        return False, f"Error checking IAM role: {e}"

    return has_ecs_permission, None
    ...
    try:
        ...
        if not targets:
            ...
            for target in targets:
                ...
                has_permission, error = check_eventbridge_ecs_permission(role_arn)
                print("\n=== 📢 FINAL CHECK ===")
                if error:
                    print(f"❌ {error}")
                elif has_permission:
                    ...
                    print("❌ EventBridge CANNOT invoke this ECS task — missing ecs:RunTask permission.")

    except botocore.exceptions.ClientError as e:
        print(f"❌ Could not retrieve rule '{EVENTBRIDGE_RULE_NAME}': {e}")
